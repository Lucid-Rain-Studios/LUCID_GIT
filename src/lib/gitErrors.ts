...
const DEFS: ErrorDef[] = [
  ...
  },
  {
    ...
    causes: [
      ...
      'Antivirus or a sync client (OneDrive, Dropbox) is holding files under .git — this also slows git enough that its own commands start overlapping',
      'A long Lucid Git operation (fetch, checkout, staging a large changeset) is still finishing in the background',
    ],
    severity: 'error',
    canAutoFix: false,
    fixes: [
